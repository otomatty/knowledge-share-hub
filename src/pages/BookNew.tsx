...
export default function BookNew() {
  ...
  const availableArticles = mockArticles.filter(a => !selectedArticles.includes(a.id));
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="space-y-6">
          <div className="space-y-2">
            <Label>タイトル</Label>
            <Input value={title} onChange={e => setTitle(e.target.value)} placeholder="ブックのタイトル" className="text-xl h-12" />
          </div>
          <div className="space-y-2">
            <Label>説明</Label>
            <Textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="ブックの説明文..." className="h-20 resize-none" />
          </div>
          <div className="space-y-2">
            <Label>カバー画像</Label>
                  ...
                  return (
                    <Card key={articleId}>
                      <CardContent className="p-3 flex items-center gap-3">
                        ...
                        <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setSelectedArticles(prev => prev.filter(id => id !== articleId))}>
                          <X className="h-4 w-4" />
                        </Button>
                      </CardContent>
                    </Card>
              ...
              <div className="border rounded-lg p-3 space-y-1">
                ...
                {availableArticles.map(article => (
                  <button
                    ...
                    onClick={() => setSelectedArticles(prev => [...prev, article.id])}
                  >
          ...
          <div className="flex gap-2">
            ...
            <Button disabled={!title.trim() || selectedArticles.length === 0}>公開する</Button>
          </div>
        </div>
      </div>
    </MainLayout>
